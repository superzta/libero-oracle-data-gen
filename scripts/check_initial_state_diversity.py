...
_THRESHOLDS = {
    ...
    "diverse_v2":  {
        "min_x_range": 0.120,       # 12 cm cube x range
        "min_y_range": 0.080,       # 8 cm cube y range
        "min_std": 0.035,
        "min_bins": 6,              # positional grid occupancy (100-demo)
        "min_bins_5demo": 5,        # positional grid occupancy (5-demo gate; can't fill 6 cells with 5 demos)
        "min_cube_named_bins": 6,   # named bin IDs (out of 6) for 100 demos
        "min_box_named_bins": 3,    # named bin IDs (out of 4) for 100 demos
        "min_cube_named_bins_5demo": 3,  # for 5-demo visual gate (5 draws can't cover 6 cells)
        "min_box_named_bins_5demo": 2,   # for 5-demo visual gate
        "min_unique_seeds": 5,      # at least 5 unique seeds (100 for full run)
...
def main() -> None:
    ...
    is_5demo_bins = n_episodes <= 10 and "min_bins_5demo" in thresholds
    min_bins_req = thresholds["min_bins_5demo"] if is_5demo_bins else thresholds["min_bins"]
    if bins_occupied < min_bins_req:
    ...
    # diverse_v2 named-bin checks
    if level == "diverse_v2" and any(b >= 0 for b in cube_bin_ids):
